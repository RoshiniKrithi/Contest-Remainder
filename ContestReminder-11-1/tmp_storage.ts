...
export class MemStorage implements IStorage {
  ...
  private async initializeAdminUser() {
    ...
    console.log("Γ£à Default admin user created (username: admin, password: admin123)");
  }
...
export class DatabaseStorage implements IStorage {
  ...
  async seedChallengeData() {
    try {
      ...
      if (existingTyping.length === 0) {
        console.log("≡ƒî▒ Seeding Typing Challenges...");
      ...
      if (existingQuiz.length === 0) {
        console.log("≡ƒî▒ Seeding Quiz Questions...");
      ...
      if (existingTeasers.length === 0) {
        console.log("≡ƒî▒ Seeding Brain Teasers...");
  ...
  private async initializeAdminUser() {
    try {
      ...
      console.log("Γ£à Default admin user created (admin / admin123)");
    } catch (err) {
  ...
  async autoPatchDatabase() {
    try {
      console.log("≡ƒôí Intelligence Sync: Initializing...");
      // Small delay to ensure DB connection is stable
      await new Promise(resolve => setTimeout(resolve, 3000));

      const existingLessons = await this.db.select().from(lessons);
      console.log(`≡ƒôí Intelligence Sync: Found ${existingLessons.length} records.`);
      ...
      for (const patch of patches) {
        ...
        if (matchingLesson) {
          console.log(`≡ƒôí Intelligence Sync: Updating mission -> ${patch.title}`);
          await this.db.update(lessons)
            .set(patch.updates)
            .where(eq(lessons.id, matchingLesson.id));
        } else if (patch.isCritical) {
          console.log(`≡ƒôí Intelligence Sync: Deploying critical mission -> ${patch.title}`);
          ...
          console.log(`≡ƒôí Intelligence Sync: Mission not found -> ${patch.title}`);
        }
      }
      console.log("≡ƒôí Intelligence Sync: Complete.");
  ...
  async seedChallenges() {
    try {
      console.log("≡ƒî▒ Challenges Sync: Initializing...");
      ...
      console.log("≡ƒî▒ Challenges Sync: Complete.");
    } catch (error) {
