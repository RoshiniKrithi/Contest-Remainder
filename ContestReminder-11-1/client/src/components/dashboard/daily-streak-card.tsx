...
export default function DailyStreakCard() {
    const { data: daily, isLoading } = useQuery<DailyChallengeData>({
        ...
        staleTime: 5 * 60 * 1000,
        // Don't block render — load in background
        refetchOnMount: true,
    });
        ...
        >
            ...
            <Card className="relative bg-slate-900/90 border-white/5 backdrop-blur-xl overflow-hidden rounded-3xl">
                <CardContent className="p-6">
                    {isLoading ? (
                        /* Compact inline skeleton — doesn't block layout */
                        ...
                        <div className="text-slate-500 text-sm text-center py-4">No daily challenge available</div>
                    ) : (
                        <>
                            <div className="flex items-start justify-between mb-6">
                                <div>
                                    ...
                                    <h3 className="text-lg font-black text-white leading-tight">{daily.title}</h3>
                                </div>
                            ...
                            <div className="space-y-4">
                                <div className="flex items-end justify-between">
                                    <div>
                                        <p className="text-[10px] uppercase font-bold text-slate-500 tracking-wider mb-1">Current Streak</p>
                                    ...
                                    {daily.solvedToday ? (
                                        ...
                                        <Link href={`/problems/${daily.problemId}`}>
                                            <Button size="sm" className="bg-white text-slate-950 hover:bg-slate-200 font-bold text-xs uppercase tracking-wider rounded-xl h-10 px-4 shadow-lg shadow-white/5">
                                                Deploy <ArrowRight className="h-3 w-3 ml-2" />
                                            </Button>
                                        </Link>
                                    )}
                                </div>
                                <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden">
                                    <div className={`h-full rounded-full transition-all duration-1000 ${daily.solvedToday ? "bg-orange-500 w-full shadow-[0_0_10px_rgba(249,115,22,0.5)]" : "bg-slate-600 w-[5%]"}`} />
                                </div>
                            </div>
                        </>
                    )}
